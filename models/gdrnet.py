...
============================================================
Multi-modal cell/drug feature integration with DCN v2 cross network.

Architecture:
  Cell path:  gene MLP (2000→256) + scF MLP (3072→256) + cell_id_emb (64) → 576
  Drug path:  FP MLP (2048→256) + drug_id_emb (64) → 320
  Interaction: concat (896) → project (512) → DCN v2 cross + Deep MLP → predict
"""
...
class GDRNet(nn.Module):
    ...
    def __init__(
        ...
        # ── Drug encoders (FP + ID only, no desc) ──
        ...
        drug_dim = d_hidden + id_emb_dim        # 256 + 64 = 320
        combined = cell_dim + drug_dim           # 896
    ...
    def forward(self, x_gene, scf, x_fp, x_desc, cell_idx, drug_idx):
        ...
        # Drug (FP + ID only, x_desc ignored)
...
class GDRNetDataset(Dataset):
    def __init__(self, x_gene, scf, x_fp, x_desc, cell_idx, drug_idx, y):
        ...
        # x_desc kept for interface compatibility but not used
        self.x_desc = torch.zeros(len(y), 1)  # dummy
